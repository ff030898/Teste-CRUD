    ...
    <v-app id="inspire">
      ...
      <v-toolbar app dark>
        <v-toolbar-title class="headline text-uppercase">
          <span>CRUD - </span>
          <span class="font-weight-light">Folha de Ponto</span>
        </v-toolbar-title>
        <v-spacer></v-spacer>
        <p class="ma-4"><b>Funcionário: </b> {{ username }}</p>
      </v-toolbar>
        ...
        <v-toolbar flat color="white">
          ...
          <v-dialog v-model="dialog" max-width="500px">
            <template v-slot:activator="{ on }">
              <v-btn v-show="enabled" color="primary" dark class="mb-2" @click="createAssignment()">Registrar
                ponto</v-btn>
            </template>
                ...
                <v-container grid-list-md>
                  <v-layout wrap>
                    <v-flex xs12 sm6 md4 v-if="editedItem.open !== 0">
                      <v-text-field v-model="editedItem.open" label="Hora de entrada" type="time"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6 md4 v-if="editedItem.interval1 !== 0">
                      <v-text-field v-model="editedItem.interval1" label="Início almoço" type="time"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6 md4 v-if="editedItem.interval2 !== 0">
                      <v-text-field v-model="editedItem.interval2" label="Término almoço" type="time"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6 md4 v-if="editedItem.exit !== 0">
                      <v-text-field v-model="editedItem.exit" label="Hora de saída" type="time"></v-text-field>
                    </v-flex>
                  </v-layout>
                </v-container>
              </v-card-text>
